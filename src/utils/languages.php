...
function printLanguages ($conn, $arr) {
    ...
    if($stmt->rowCount() > 0) {
        while($language = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            echo "<tr>";
                echo "<td data-label='Name'>{$language['name']}</td>";
                echo "<td data-label='Level'>{$language['level']}</td>";
                echo "<td class='table__options'>";
                    echo "<a href='edit_language.php?id={$language['id']}'>";
